...
def get_random_batch(batch, style_image, uncontent_flag, unstyle_flag, data_random_rate):
...
def get_canny_map(images):
    input_images = rearrange(images, 'b c h w -> b h w c').cpu().numpy()
    input_images = (input_images + 1) * 255 / 2
    control_maps = np.stack([cv2.Canny(np.uint8(images[inp]), 100, 200) for inp in range(images.shape[0])])
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    def get_all_feature(batch, style_image):
        ...
        return batch, style_image, train_content_flag, train_style_flag, latents, noise, noisy_latents, timesteps, uncond_embeddings, content_feature, style_feature, style_vae_feature, down_block_additional_residuals, full_control_image
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(content_loader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(adapter), accelerator.accumulate(discriminator), accelerator.accumulate(cooccur):
                if gan_loss_flag:
                    if use_d_count % update_dis_num == 0:
                        with accelerator.autocast():
                            batch, style_image, train_content_flag, train_style_flag, latents, noise, noisy_latents, timesteps, uncond_embeddings, content_feature, style_feature, style_vae_feature, down_block_additional_residuals, _ = get_all_feature(batch, style_image)
                            use_d_count = 0 
                ...
                with accelerator.autocast():
                    batch, style_image, train_content_flag, train_style_flag, latents, noise, noisy_latents, timesteps, uncond_embeddings, content_feature, style_feature, style_vae_feature, down_block_additional_residuals, full_control_image = get_all_feature(batch, style_image)
                    ...
                    if all_loss_type == 'latent':
                        ...
                        for iii in range(len(control_mode)):
                            control_this_mode = control_mode[iii]
                            this_annotator_model = annotator_model[iii]
                            if control_this_mode == 'hed':
                                print(i, 'hed_out')
                                control_image_out = get_hed_map(this_annotator_model, image_out)
                            elif control_this_mode == 'depth':
                                print(i, 'depth_out')
                                control_image_out = get_depth_map(this_annotator_model, image_out, train_data.height, train_data.width, return_standard_norm=False)
                            elif control_this_mode == 'seg':
                                print(i, 'seg_out')
                                control_image_out = get_seg_map(seg_model, image_out).to(weight_dtype).to(batch.device)
                            elif control_this_mode == 'canny':
                                print(i, 'canny_out')
                                control_image_out = get_canny_map(image_out)
                            elif control_this_mode == 'style':
                                print(i, 'style_out')
                                control_image_out = style_image
                            else:
                                print(i, 'else_out')
                                control_image_out = batch
                            control_loss = control_loss + F.mse_loss(control_image_out, full_control_image[iii])
                        

                    loss = content_loss_weight * target_mse_loss + style_loss_weight * style_loss + gan_loss_weight * g_loss + patch_gan_loss_weight * g_cooccur_loss + control_weight * control_loss
                ...
                avg_control_loss = accelerator.gather(control_loss.repeat(train_batch_size)).mean()
                train_control_loss += avg_control_loss.item() / gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                global_step += 1
                accelerator.log(
                    {"train_loss": train_loss, "train_c_loss": train_c_loss, "train_s_loss": train_s_loss, "train_g_loss": train_g_loss, "train_co_loss": train_co_loss, "train_d1_loss": train_d1_loss, "train_d2_loss": train_d2_loss, "train_control_loss": train_control_loss}, 
                    step=global_step
                )
                train_loss = 0.0
                train_c_loss = 0.0
