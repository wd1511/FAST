...
class SpatioTemporalTransformerBlock(nn.Module):
    def __init__(
        ...
        if cross_attention_dim is not None:
            # print(f"num:{num_attention_heads} dim:{attention_head_dim}")
    ...
    def forward(
        ...
        if self.attn2 is not None:
            ...
            if self.use_gamma:
                ...
                # print(f"hd:{norm_hidden_states.shape} enc:{encoder_hidden_states.shape}")
                hidden_states = (
                    self.attn2(
                        norm_hidden_states,
                        encoder_hidden_states=encoder_hidden_states,
...
class SparseCausalFullAttention(CrossAttention):
    def forward(
        ...
        hidden_states = xformers.ops.memory_efficient_attention(
            query.contiguous(), key.contiguous(), value.contiguous(), attn_bias=attention_mask, op=None
        )
        hidden_states = hidden_states.to(query.dtype)
        hidden_states =  self.batch_to_head_dim(hidden_states)
