...
from model.diffusion.models.unet_2d_blocks import (
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        self.content_in = nn.Sequential(
            nn.Conv2d(content_channels, content_refined_channels, 1),
            nn.SiLU(),
            nn.Conv2d(content_refined_channels, content_refined_channels, 1),
        )
        ...
        self.conv_in_new = nn.Conv2d(
            in_channels+self.content_refined_channels, block_out_channels[0], kernel_size=conv_in_kernel, padding=conv_in_padding
        )
    ...
    def get_content_feature(self, content_image, vae, flag=None):
        # import pdb; pdb.set_trace()
        features = vae.encode(content_image).latent_dist.sample()
    ...
    def forward(
        ...
        if content != None:
            ...
            content_input = self.content_in(modulate(content_input, shift, scale))
    ...
    @classmethod
    def from_2d_model(cls, model_path, use_pretrained=True, condition_on_fps=False):
    ...
    def load_2d_state_dict(self, state_dict, **kwargs):
        ...
        for k, v in state_dict.items():
            if k not in state_dict_2d:
                raise KeyError(f"2d state_dict key {k} does not exist in 3d model")
            load_state_dict[k] = v


        state_dict_2d.update(load_state_dict)
        self.load_state_dict(state_dict_2d, **kwargs)

    def input_layer_init(self):
        nn.init.zeros_(self.conv_in_new.weight)
        nn.init.zeros_(self.conv_in_new.bias)
        self.conv_in_new.weight.data[:, :4, :, :] = self.conv_in.weight.data.clone()
        # import pdb; pdb.set_trace()
        self.conv_in_new.bias.data = self.conv_in.bias.data.clone()
