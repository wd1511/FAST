...
from model.diffusion.models.adapter import MultiAdapter, T2IAdapter, StyleAdapter
...
class StableDiffusionAdapterPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        adapter: Union[T2IAdapter, MultiAdapter, StyleAdapter, List[T2IAdapter]],
    ...
    @torch.no_grad()
    def get_hed_map(self, images, num):
        ...
        out_edge[:, 1, :, :] = edge[:, 0, :, :]
        out_edge[:, 2, :, :] = edge[:, 0, :, :]
        # out_edge = F.interpolate(out_edge, size=(h, w), mode='bilinear', align_corners=False)
        return out_edge
    
    @torch.no_grad()
    def get_seg_map(self, images):
        dtype = images.dtype
    ...
    def get_depth_map(self, images, height, width, num, return_standard_norm=False):
        h,w = height, width
    ...
    def __call__(
        ...
        if not adapter_cfg:
            ...
            if do_classifier_free_guidance and content != None and style != None:
                for k, v in enumerate(adapter_state):
                    adapter_state[k] = torch.cat([v] * 3, dim=0)
            elif do_classifier_free_guidance and content != None:
                for k, v in enumerate(adapter_state):
                    adapter_state[k] = torch.cat([v] * 2, dim=0)
        else:
            num_adapter = len(adapter_conditioning_scale)
            adapter_state_list = []
            for i in range(num_adapter):
                this_weight = [0] * num_adapter
                this_weight[i] = adapter_conditioning_scale[i]
                adapter_state = self.adapter(adapter_input, this_weight)
                for k, v in enumerate(adapter_state):
                    adapter_state[k] = v
                if do_classifier_free_guidance and content != None and style != None:
                    for k, v in enumerate(adapter_state):
                        adapter_state[k] = torch.cat([v] * 3, dim=0)
                adapter_state_list.append(adapter_state)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if adapter_cfg:
                    ...
                    noise_sum = 0
                    for jjj in range(len(adapter_conditioning_scale)):
                        noise_pred_all = self.unet(
                            ...
                            down_intrablock_additional_residuals=[state.clone() for state in adapter_state_list[jjj]],
                        ).sample
                        ...
                        noise_sum = noise_sum + noise_pred
                    noise_pred = noise_sum / len(adapter_conditioning_scale)
                else:
                    if do_classifier_free_guidance and content != None and style != None:
                        latent_model_input_all = torch.cat([latent_model_input, latent_model_input, latent_model_input])
                        content_feature_all = torch.cat([content_feature, content_feature, null_content_feature])
                        ...
                        latent_model_input_all = torch.cat([latent_model_input, latent_model_input])
                        content_feature_all = torch.cat([content_feature, null_content_feature])
                        style_feature_all = None
                        prompt_embeds_all = torch.cat([prompt_embeds, prompt_embeds])
                        if use_adapter:
                            noise_pred_all = self.unet(
                                latent_model_input_all,
                                t,
                        ...
                        noise_pred, noise_pred_uncontent = noise_pred_all.chunk(2)
                        noise_pred = noise_pred_uncontent * (1-noise_pred_uncontent) + noise_pred * noise_pred_uncontent
                    else:
                        if use_adapter:
                            noise_pred_all = self.unet(
                                latent_model_input_all,
